...
export const useSubjectService = () => {
    ...
    const [ singleSubjectLoading, setSingleSubjectLoading ] = useState<boolean>(false);
    ...
    const getSubjectByExam = async (id: string | undefined) => {
        try{
            ...
            const errorMessage =
                error.response?.data?.message || error.message || "Could not create subjects";
                setServerError(errorMessage);
            toast.error(errorMessage)
        }
    };

    const editSubject = async ({id, data} : { id: string | undefined, data: SubjectFormData }) => {
        setServerError("")
        ...
        try{
            ...
            setSingleSubjectLoading(false)
            const errorMessage =
                error.response?.data?.message || error.message || "Could not updated subject";
                setServerError(errorMessage);
            toast.error(errorMessage)
        }
    }
    ...
    const createSubject = async (data: SubjectFormData) => {
        try{
            ...
            const errorMessage =
                error.response?.data?.message || error.message || "Could not add subject";
                setServerError(errorMessage);
            toast.error(errorMessage)
        }
    };
    ...
    const deleteSubject = async (id:string | undefined) => {
        try{
            ...
            const errorMessage =
                error.response?.data?.message || error.message || "Could not delete subject";
                setServerError(errorMessage);
            toast.error(errorMessage)
        }
    }
    ...
    const getSingleSubject = async (subjectId: string | undefined) => {
        try{
            ...
            setSingleSubjectLoading(false)
            const errorMessage =
                error.response?.data?.message || error.message || "Could not get subject";
                setServerError(errorMessage);
                toast.error(errorMessage)
        }
    }
