...
export const useExamTypeService = () => {
  ...
  const [singleExamTypeLoading, setSingleExamTypeLoading] =
    useState<boolean>(false);
  const [serverError, setServerError] = useState<string | null>(null);
  const router = useRouter();

  const { data, error, isLoading, mutate } = useSWR<ExamDataTypes>(
    `/exam-type/organization/${organization?.id}`,
    fetcher
  );
  ...
  const editExamType = async ({
    id,
    data,
  }: {
    id: string | undefined;
    data: ExamTypeFormData;
  }) => {
    setServerError('');
    setSingleExamTypeLoading(true);
    try {
