...
export const useExamModeResultService = () => {
  ...
  const [examModeResult, setExamModeResult] =
    useState<ExamModeResultType | null>(null);
  const [singleExamModeResultLoading, setSingleExamModeResultLoading] =
  ...
  const { data, error, isLoading, mutate } = useSWR<
    ExamModeResultMainDataType | undefined
  >(`/exam-mode-result/${organization?.id}`, fetcher);
