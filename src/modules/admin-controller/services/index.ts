...
import { PasswordResetFormData, passwordResetSchema } from "@/modules/password-reset/schema/passwordResetSchema";
import { TwoFactorFormData, twoFactorSchema } from "@/modules/2fa/schema/2faSchema";
import { useAltStore } from "@/lib/zustand/userStore";
import { ForgetPasswordFormData, forgetPasswordSchema } from "@/modules/forget-password/schemas/forgetPasswordSchema";
  ...
  const forgetPassword = async (data: ForgetPasswordFormData) => {
    ...
    const passwordReset = async (data: PasswordResetFormData) => {
        ...
        try{
            ...
            const errorMessage =
                err.response?.data?.message || err.message || "Failed to sent password reset link";
            setServerError(errorMessage);
            toast.error(errorMessage);
        }
    };
