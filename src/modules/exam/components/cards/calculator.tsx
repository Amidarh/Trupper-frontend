...
export function Calculator({
    ...
    return combined.flat().map((btn) => (
      <button
        ...
        className={`p-2 rounded-xl border cursor-pointer font-medium ${
          btn === '=' ? 'bg-green-600 hover:bg-green-700 col-span-1' : ''
        }`}
      >
        {btn}
      </button>
  ...
  return (
    <>
      ...
      <main
        className={cn(
          'border rounded-xl top-22 right-5 flex items-center justify-center p-4 z-10',
          isOpen ? 'fixed' : 'hidden',
          bgBlur
        )}
      >
        <div className='w-[280px]'>
          <div className='flex justify-end items-center mb-2'>
            ...
            <div>
              <X onClick={onToggle} className='cursor-pointer' />
            </div>
          </div>
