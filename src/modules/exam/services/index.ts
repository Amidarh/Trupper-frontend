...
import {
  EditExamCategoryFormData,
  editExamCategorySchema,
  ExamCategoryFormData,
  examCategorySchema,
} from '../schema/examCategory';
...
export const useExamService = () => {
  ...
  const [examCategoryList, setExamCategoryList] = useState<
    ExamCategoryType[] | null
  >(null);
  const [examCategoryLoading, setExamCategoryLoading] =
    useState<boolean>(false);
  const { id } = useParams<{ id: string | undefined }>();
  const [examCategory, setExamCategory] = useState<ExamCategoryType | null>(
    null
  );
  // const [ exam ]
  const router = useRouter();

  const { data, error, isLoading, mutate } = useSWR<ExamDataType>(
    `/exams/organization/${organization?.id}`,
    fetcher
  );
  ...
  const deleteExam = async (id: string | undefined | null) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        'Could not delete this exam type';
      setServerError(errorMessage);
      toast.error(errorMessage);
    }
  };
  ...
  const createExamCategory = async (data: ExamCategoryFormData) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        'Could not create exam category';
      setServerError(errorMessage);
      toast.error(errorMessage);
    }
  };
  ...
  const editExamCategory = async (
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        'Could not update this exam categories';
      setServerError(errorMessage);
      toast.error(errorMessage);
    }
  };
  ...
  const removeExamCategory = async (id: string) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        'Could not remove this exam categories';
      setServerError(errorMessage);
      toast.error(errorMessage);
    }
  };
