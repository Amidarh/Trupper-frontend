...
export function useExamService(){
    ...
    const getSkippedQuestions = examState?.questions.filter((el) => el.userAnswer === '');
    ...
    failedQuestions = (examState?.questions.length ?? 0) - (getPassedQuestions?.length ?? 0);
    examPercentage = Math.floor(
        ((getPassedQuestions?.length ?? 0) / (examState?.questions.length ?? 0)) * 100
    );
