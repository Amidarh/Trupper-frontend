...
export const SubjectCard = ({
  ...
  action: (data: ExamCategoryType) => void;
  selected: string;
}) => {
  return (
    <Card
      className={cn(
        'py-2 px-4 gap-4 flex-row cursor-pointer items-center text-ellipsis truncate',
        data.id === selected &&
    ...
    >
      <div
        className={cn(
          'size-4 min-w-4 h-4 border rounded-full',
          data.id === selected && 'bg-gray-900 dark:bg-white'
        )}
      />
      <div>
        <h1>{name}</h1>
