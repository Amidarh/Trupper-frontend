  ...
  useEffect(() => {
    if(singleQuestion && exams){
      getSubjectByExam(singleQuestion.exam.id)
        ...
        setSelectedSubject(subjectList?.find(subject => subject.id === singleQuestion.subject.id))
        setSelectedQuestionType(questionTypeData.find(questionType => questionType.name === singleQuestion.questionType))
    }
      ...
      <form className="py-0 px-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div>
            ...
            >
              <SelectTrigger className="w-full h-12">
                <SelectValue placeholder={examLoading ? "Loading..." : singleQuestion?.exam.name || "Select exam"} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  {exams?.map((exam) => (
          ...
          <div>
            ...
            >
              <SelectTrigger className="w-full h-12">
                <SelectValue placeholder={singleQuestion?.subject.name || "Select subject"} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
          ...
          <div>
            ...
            >
              <SelectTrigger className="w-full h-12">
                <SelectValue placeholder={ selectedQuestionType?.name ?? singleQuestion?.questionType ?? "Select question type"} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
          ...
          <div>
            ...
            >
              <SelectTrigger className="w-full h-20">
                <SelectValue placeholder={ singleQuestion?.answer ?? "Select answer"} />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <RichTextEditor content={section} onChange={(e) => setValue("section", e, { shouldValidate: true })} disabled={!edit}
            />
        ...
        <div className="mb-4">
          ...
          <RichTextEditor content={singleQuestion?.question || question} onChange={(e) => setValue("question", e, { shouldValidate: true })}
            disabled={!edit}
          />
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div>
            ...
            <RichTextEditor content={a} onChange={(e) => setValue("a", e, { shouldValidate: true })} disabled={!edit} />
            {errors.a && <p className="text-red-500 text-sm">{errors.a.message}</p>}
          </div>

          <div>
            <Label className="mb-2">Option B</Label>
            <RichTextEditor content={b} onChange={(e) => setValue("b", e, { shouldValidate: true })} disabled={!edit} />
            {errors.b && <p className="text-red-500 text-sm">{errors.b.message}</p>}
          </div>

          <div>
            <Label className="mb-2">Option C</Label>
            <RichTextEditor content={c} onChange={(e) => setValue("c", e, { shouldValidate: true })} disabled={!edit} />
            {errors.c && <p className="text-red-500 text-sm">{errors.c.message}</p>}
          </div>

          <div>
            <Label className="mb-2">Option D</Label>
            <RichTextEditor content={d} onChange={(e) => setValue("d", e, { shouldValidate: true })} disabled={!edit} />
