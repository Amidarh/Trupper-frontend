...
export const useQuestionService = () => {
    ...
    const [ singleQuestion, setSingleQuestion ] = useState<QuestionType | null>(null);
    const [ singleQuestionLoading, setSingleQuestionLoading ] = useState<boolean>(false);
    const [ serverError, setServerError ] = useState<string | null>(null)
    const router = useRouter();

    const { data, error, mutate, isLoading } = useSWR<QuestionDataType>(`/questions/organization/${organization?.id}`, fetcher)
    ...
    const createQuestion = async (data: FormData) => {
        try{
            ...
            const errorMessage =
                error.response?.data?.message || error.message || "Could not create question";
    ...
    const deleteQuestion = async (id:string | undefined) => {
        try{
            ...
            const errorMessage =
                error.response?.data?.message || error.message || "Could not delete this question";
            setServerError(errorMessage);
            toast.error(errorMessage)
        }
    }
    ...
    const editQuestion = async (id: string | undefined, data: QuestionFormData) => {
        ...
        try{
            ...
            setSingleQuestionLoading(false)
            const errorMessage =
                error.response?.data?.message || error.message || "Could not update this question";
            setServerError(errorMessage);
            toast.error(errorMessage)          
        }
    }
