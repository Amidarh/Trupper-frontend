    ...
    const kycComplete = async (data: SubAdminKycSchemaFormData) => {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                document.cookie = `role=${(admin.role).toUpperCase()}; path=/; secure; SameSite=Strict`;
                document.cookie = `organizationId=${admin.organization?.id || "default"}; path=/; secure; SameSite=Strict`;
                setUser(admin);
