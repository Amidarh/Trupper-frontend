            ...
            <TableBody>
              {data?.map(mode => (
                  ...
                  >
                      ...
                      <TableCell>{moment(mode.createdAt).format('MMMM D, YYYY')}</TableCell>
                      <TableCell>{mode.exam.noOfQuestions}</TableCell>
                      <TableCell>
                          {getStatusBadge( mode.status ? 'active' : "inactive")}
                      </TableCell>
                      <TableCell className='flex justify-end items-center gap-2'>
                          <EnableExamModeModal id={mode.id} validFrom={mode.validFrom} validTill={mode.validTill} status={mode.status}/>
