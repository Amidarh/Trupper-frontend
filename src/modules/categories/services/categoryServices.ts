...
export const useCategoryService = () => {
  ...
  const [singleCategory, setSingleCategory] = useState<CategoryTypes | null>(
    null
  );
  const [singleCategoryLoading, setSingleCategoryLoading] =
    useState<boolean>(false);
  const [serverError, setServerError] = useState('');
  const router = useRouter();

  const { data, error, isLoading, mutate } = useSWR<CategoryDataTypes>(
    `/category/category-one-by-organization/${organization?.id}`,
    fetcher
  );
