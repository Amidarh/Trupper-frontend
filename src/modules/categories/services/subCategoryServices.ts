...
export const useSubCategoryService = () => {
  ...
  const [singleSubCategory, setSingleSubCategory] =
    useState<SubCategoryTypes | null>(null);
  const [singleSubCategoryLoading, setSingleSubCategoryLoading] =
  ...
  const { data, error, isLoading, mutate } = useSWR<SubCategoryDataTypes>(
    `/category/category-two-by-organization/${organization?.id}`,
    fetcher
  );
