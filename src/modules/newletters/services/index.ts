...
export const useNewsletterService = () => {
  ...
  const [singleNewsletter, setSingleNewsletter] =
    useState<NewsletterType | null>(null);
  const [singleNewsletterLoading, setSingleNewsletterLoading] =
    useState<boolean>(false);
  const [serverError, setServerError] = useState<string | null>(null);
  const router = useRouter();

  const { data, error, isLoading, mutate } = useSWR<NewsletterDataType>(
    `/newsletter/organization/${organization?.id}`,
    fetcher
  );
  ...
  const createAndDraftNewsletter = async (data: NewsletterFormData) => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.message ||
        'Could not create as save newsletter';
  ...
  const createAndSend = async (data: NewsletterFormData) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message ||
        error.message ||
        'Could not create and send newsletter';
      setServerError(errorMessage);
      toast.error(errorMessage);
    }
  };
